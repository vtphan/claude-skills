...
9. **Code-quality lens (condition-triggered).** Apply by default whenever the diff includes non-trivial logic in a load-bearing module — i.e., the diff is not purely config, schema, docs, comments, formatting, or plumbing-only changes (rename, reformat, dependency bump, fixture rearrangement). When the condition fires, surface findings under five sub-lenses, each as its own finding with severity and evidence: (a) coupling and duplication that crossed module boundaries, (b) error handling — paths that swallow errors or assume success, (c) edge cases the diff didn't account for, (d) test quality — happy-path-only coverage, fixtures hiding the actual assertion, (e) accepted debt the executor took without flagging in the report. Skip this category only with an explicit one-line reason in the findings doc (e.g., "skipped: diff is config-only"). The goal is to catch what plan-conformance review wouldn't — not a full code review, but the five lenses applied with judgment. If the human asks for a deeper code review at wave-update time, hand off to a separate review skill rather than expanding this category.
