...
Checkbox state (`[ ]` / `[x]`) is the single source of truth for task status. The executor skill is the only skill that flips checkboxes, and does so as it completes each task. A partially-complete task stays `[ ]` and is explained in the report's "Task status" section with a `[~]` notation there. Once a phase closes out (via the updater), the task list is replaced by the closeout summary, so the checkboxes' role is bounded to the current phase.
...
8. **Only the updater skill makes structural changes to the plan.** The executor is permitted exactly one kind of modification — ticking task checkboxes `[ ]` → `[x]` as it completes them. Everything else (adding or removing tasks, editing sections, changing phase status, updating registers, writing the change log) goes through the report → updater pipeline. This keeps the audit trail clean: any structural change to the plan is accompanied by a corresponding change-log entry the updater wrote.
